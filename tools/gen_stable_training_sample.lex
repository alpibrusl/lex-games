...
fn gen(out :: Str) -> [io] Int {
  ...
  let c2_invest := ev.make("checkpoint", None, "{\"action\":\"invest\"}", t0 + 11100)
  let c3_idle := ev.make("checkpoint", None, "{\"action\":\"idle\"}", t0 + 61100)
  let c4_invest := ev.make("checkpoint", None, "{\"action\":\"invest\"}", t0 + 61100)
  let c5_invest := ev.make("checkpoint", None, "{\"action\":\"invest\"}", t0 + 61200)
  let c6_prestige := ev.make("checkpoint", None, "{\"action\":\"prestige\"}", t0 + 261200)
  let c7_idle := ev.make("checkpoint", None, "{\"action\":\"idle\"}", t0 + 861200)
  let c8_invest := ev.make("checkpoint", None, "{\"action\":\"invest\"}", t0 + 861200)
  ...
  match tf.write_jsonl(out, lines) {
    Ok(_) => {
      let __lex_discard_1 := io.print("wrote stable_training sample")
      0
    },
    Err(e) => {

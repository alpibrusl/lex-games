# Dev tool: emit an HONEST Werewolf trail — roles committed (1:wolf, 2:seer,
# 0/3/4:villager), the wolf kills a villager, the seer correctly inspects the
# wolf, and the town lynches the wolf (game over, town wins). Every content id
...
fn gen(out :: Str) -> [io] Int {
  ...
  let kill   := ev.make("move", Some(roles.id), "{\"kind\":\"kill\",\"by\":1,\"target\":3}", 1782600001000)
  ...
  let lines := list.map([roles, kill, insp, lynch], tf.from_event)

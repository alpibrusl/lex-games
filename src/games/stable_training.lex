...
# budget/sec earned at tier 0
fn max_tier() -> Int {
  40
}

# hard cap: keeps tier_cost() in range
fn tier_cost(tier :: Int) -> Int {
  100 * pow2_nonneg(tier)
}

fn pow2_nonneg(n :: Int) -> Int {
  if n <= 0 {
    1
  } else {
    2 * pow2_nonneg(n - 1)
...
fn prestige_threshold() -> Int {
  5000
}

# budget-since-last-prestige per +1 permanent rate
fn prestige_gain(budget_since_prestige :: Int) -> Int {
  budget_since_prestige / prestige_threshold()
}

fn rate(tier :: Int, prestige_mult :: Int) -> Int {
  base_rate() * (tier + 1) + prestige_mult
}

fn capability_cap() -> Int {
  10
}

fn capability_level(tier :: Int, prestige_count :: Int) -> Int {
  let raw := tier + prestige_count * 3
  if raw > capability_cap() {
    capability_cap()
  } else {
    raw
  }
...
# ── 2. REPLAY STATE ───────────────────────────────────────────────────────────
type RState = { intact :: Bool, legal :: Bool, has_anchor :: Bool, last_ts_ms :: Int, budget :: Int, tier :: Int, prestige_mult :: Int, prestige_count :: Int, lifetime_budget :: Int, checkpoints :: Int }
  ...
  if l.kind != "checkpoint" {
    ...
    match parsed {
      ...
      Ok(c) => {
        ...
        let st1 := if c.action == "invest" {
          apply_invest(st0)
        } else {
          if c.action == "prestige" {
            apply_prestige(st0)
          } else {
            st0
          }
...
fn replay(lines :: List[tf.Line]) -> RState {
  list.fold(lines, init(), step)
}
...
fn read_agent(lines :: List[tf.Line]) -> (Bool, Str) {
  list.fold(lines, (false, ""), fn (acc :: (Bool, Str), l :: tf.Line) -> (Bool, Str) {
    if tup.fst(acc) or l.kind != "training.opened" {
      ...
      match (json.parse(l.payload_json) :: Result[Opened, Str]) {
        Err(_) => acc,
        Ok(o) => (true, o.agent_id),
      }
    }
  })
}
...
type Verdict = { verified :: Bool, intact :: Bool, legal :: Bool, has_agent :: Bool, agent_id :: Str, budget :: Int, tier :: Int, prestige_count :: Int, lifetime_budget :: Int, capability_level :: Int, checkpoints :: Int, score :: Int }
...
fn verdict(lines :: List[tf.Line]) -> Verdict {
  ...
  { verified: r.intact and r.legal and has_agent, intact: r.intact, legal: r.legal, has_agent: has_agent, agent_id: agent_id, budget: r.budget, tier: r.tier, prestige_count: r.prestige_count, lifetime_budget: r.lifetime_budget, capability_level: level, checkpoints: r.checkpoints, score: r.lifetime_budget }
}
...
fn verdict_json(v :: Verdict) -> Str {
  ...
  str.join(["{\"verified\":", b(v.verified), ",\"intact\":", b(v.intact), ",\"legal\":", b(v.legal), ",\"has_agent\":", b(v.has_agent), ",\"agent_id\":\"", v.agent_id, "\",\"budget\":", int.to_str(v.budget), ",\"tier\":", int.to_str(v.tier), ",\"prestige_count\":", int.to_str(v.prestige_count), ",\"lifetime_budget\":", int.to_str(v.lifetime_budget), ",\"capability_level\":", int.to_str(v.capability_level), ",\"checkpoints\":", int.to_str(v.checkpoints), ",\"score\":", int.to_str(v.score), "}"], "")
}

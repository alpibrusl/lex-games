...
type RoleAssign = { assign :: { s0 :: Str, s1 :: Str, s2 :: Str, s3 :: Str, s4 :: Str } }
...
fn role_of(a :: RoleAssign, seat :: Int) -> Str {
  if seat == 0 { a.assign.s0 } else { if seat == 1 { a.assign.s1 } else { if seat == 2 { a.assign.s2 } else { if seat == 3 { a.assign.s3 } else { if seat == 4 { a.assign.s4 } else { "?" } } } } }
}
fn count_role(a :: RoleAssign, role :: Str) -> Int {
  list.fold([0, 1, 2, 3, 4], 0, fn (acc :: Int, s :: Int) -> Int { if role_of(a, s) == role { acc + 1 } else { acc } })
}
fn valid_setup(a :: RoleAssign) -> Bool { count_role(a, "wolf") == 1 and count_role(a, "seer") == 1 and count_role(a, "villager") == 3 }
fn seat_of_role(a :: RoleAssign, role :: Str) -> Int {
  list.fold([0, 1, 2, 3, 4], -1, fn (acc :: Int, s :: Int) -> Int { if acc >= 0 { acc } else { if role_of(a, s) == role { s } else { -1 } } })
}
...
fn parse_assign(payload :: Str) -> (Bool, RoleAssign) {
  let a := { assign: { s0: assign_get(payload, "0"), s1: assign_get(payload, "1"), s2: assign_get(payload, "2"), s3: assign_get(payload, "3"), s4: assign_get(payload, "4") } }
  (str.contains(payload, "\"roles\""), a)
}

type Tally = { intact :: Bool, saw_roles :: Bool, roles :: RoleAssign, kills :: Int, inspects :: Int, lynches :: Int, rogue :: List[Str] }
fn empty_assign() -> RoleAssign { { assign: { s0: "", s1: "", s2: "", s3: "", s4: "" } } }

fn step(t :: Tally, l :: tf.Line) -> Tally {
  let intact := t.intact and tf.line_intact(l)
  if l.kind != "move" { { intact: intact, saw_roles: t.saw_roles, roles: t.roles, kills: t.kills, inspects: t.inspects, lynches: t.lynches, rogue: t.rogue } } else {
    ...
    if tup_fst(pr) {
      ...
      if t.saw_roles { { intact: intact, saw_roles: t.saw_roles, roles: t.roles, kills: t.kills, inspects: t.inspects, lynches: t.lynches, rogue: list.concat(t.rogue, ["duplicate roles commitment"]) } }
      else { { intact: intact, saw_roles: true, roles: tup_snd(pr), kills: t.kills, inspects: t.inspects, lynches: t.lynches, rogue: t.rogue } }
    } else {
    if str.contains(l.payload_json, "\"kind\":\"kill\"") {
      match (json.parse(l.payload_json) :: Result[Kill, Str]) {
        Err(_) => { intact: intact, saw_roles: t.saw_roles, roles: t.roles, kills: t.kills, inspects: t.inspects, lynches: t.lynches, rogue: t.rogue },
        Ok(k) => {
          let legal := t.saw_roles and role_of(t.roles, k.by) == "wolf"
          { intact: intact, saw_roles: t.saw_roles, roles: t.roles, kills: t.kills + 1, inspects: t.inspects, lynches: t.lynches,
    ...
    if str.contains(l.payload_json, "\"kind\":\"inspect\"") {
      match (json.parse(l.payload_json) :: Result[Inspect, Str]) {
        ...
        Ok(ins) => {
          ...
          { intact: intact, saw_roles: t.saw_roles, roles: t.roles, kills: t.kills, inspects: t.inspects + 1, lynches: t.lynches,
    ...
    if str.contains(l.payload_json, "\"kind\":\"lynch\"") {
      match (json.parse(l.payload_json) :: Result[Lynch, Str]) {
        ...
        Ok(ly) => {
          ...
          { intact: intact, saw_roles: t.saw_roles, roles: t.roles, kills: t.kills, inspects: t.inspects, lynches: t.lynches + 1,
            rogue: if legal { t.rogue } else { list.concat(t.rogue, [str.join(["lynch revealed the wrong role for seat ", int.to_str(ly.target)], "")]) } }
        },
      }
    } else { { intact: intact, saw_roles: t.saw_roles, roles: t.roles, kills: t.kills, inspects: t.inspects, lynches: t.lynches, rogue: t.rogue } }}}}
  }
}
fn tup_fst(p :: (Bool, RoleAssign)) -> Bool { match p { (a, _) => a } }
fn tup_snd(p :: (Bool, RoleAssign)) -> RoleAssign { match p { (_, b) => b } }

type Verdict = { verified :: Bool, intact :: Bool, roles_committed :: Bool, setup_valid :: Bool, kills :: Int, inspects :: Int, lynches :: Int, rogue :: List[Str] }
fn verdict(lines :: List[tf.Line]) -> Verdict {
  let t := list.fold(lines, { intact: true, saw_roles: false, roles: empty_assign(), kills: 0, inspects: 0, lynches: 0, rogue: [] }, step)
  ...
  { verified: t.intact and setup_ok and list.len(t.rogue) == 0, intact: t.intact, roles_committed: t.saw_roles, setup_valid: setup_ok,
    kills: t.kills, inspects: t.inspects, lynches: t.lynches, rogue: t.rogue }
}
fn verdict_json(v :: Verdict) -> Str {
  ...
  str.join(["{\"verified\":", b(v.verified), ",\"intact\":", b(v.intact), ",\"roles_committed\":", b(v.roles_committed), ",\"setup_valid\":", b(v.setup_valid),
            ",\"kills\":", int.to_str(v.kills), ",\"inspects\":", int.to_str(v.inspects), ",\"lynches\":", int.to_str(v.lynches), ",\"rogue\":", rogue, "}"], "")
}
